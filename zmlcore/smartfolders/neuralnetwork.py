...
from neon.layers import MergeMultistream, GRU, LSTM, Affine, RecurrentSum, Tree, BranchNode, SkipNode, Conv, Pooling
from neon.initializers import GlorotUniform, Kaiming
from neon.optimizers import Adam
from neon.transforms import Softmax, Logistic, Rectlin
from ..licensed.noiselayer import Noise
    ...
    def input_layers(self, analytics_input, init, activation, gate):
        ...
        if self.recurrent:
            ...
            if analytics_input:
                ...
                input_layers = MergeMultistream([
                    [
                        Noise(noise_pct_std=1E-05),
                        Conv((1, 1, 8), padding=0, init=Kaiming(), activation=activation),
                        Conv((3, 1, 8), padding=0, init=Kaiming(), activation=activation),
                        Conv((3, 1, 8), strides={'str_h': 2, 'str_w': 1}, padding=0, init=Kaiming(),
                             activation=activation),
                        Conv((3, 1, 16), padding=0, init=Kaiming(), activation=activation),
                        Conv((3, 1, 16), strides={'str_h': 2, 'str_w': 1}, padding=0, init=Kaiming(),
                             activation=activation),
                        Conv((3, 1, 32), strides={'str_h': 2, 'str_w': 1}, padding=0, init=Kaiming(),
                             activation=activation),
                        Conv((5, 1, 64), padding=1, init=Kaiming(), activation=activation),
                        Conv((7, 3, 128), strides={'str_h': 1, 'str_w': 2}, padding=1, init=Kaiming(),
                             activation=activation),
                    ],
                ...
                input_layers = [
                    Noise(noise_pct_std=1E-05),
                    Conv((1, 1, 4), padding=0, init=Kaiming(), activation=activation),
                    Conv((3, 1, 8), padding=0, init=Kaiming(), activation=activation),
                    Conv((5, 1, 16), strides={'str_h': 2, 'str_w':1}, padding=0, init=Kaiming(), activation=activation),
                    Conv((3, 1, 24), padding=0, init=Kaiming(), activation=activation),
                    Conv((5, 1, 32), strides={'str_h': 2, 'str_w':1}, padding=0, init=Kaiming(), activation=activation),
                    Conv((3, 1, 48), padding=0, init=Kaiming(), activation=activation),
                    Conv((5, 4, 64), strides={'str_h': 1, 'str_w': 4}, padding=1, init=Kaiming(),
                         activation=activation),
                    Conv((3, 1, 96), strides={'str_h': 2, 'str_w':1}, padding=1, init=Kaiming(), activation=activation),
                    Conv((7, 5, 128), strides={'str_h': 1, 'str_w': 5}, padding=1, init=Kaiming(),
                         activation=activation),
                ]
